...
class BasicSettings(BaseFileSettings):
    ...
    def make_dirs(self):
        ...
        for p in [
            ...
            self.LOG_PATH,
            self.BASE_TEMP_DIR,
            Path(self.KB_ROOT_PATH).parent,  # 确保数据库父目录存在
        ]:
        ...
        from chatchat.server.db.models import KnowledgeBase, KnowledgeFile  # 显式导入模型类
...
class ApiModelSettings(BaseFileSettings):
    ...
    USE_XINFERENCE: bool = Field(default=False, description="是否使用 xinference")
    XINFERENCE_MODEL: str = Field(default="chatglm3", description="xinference 使用的模型名称")
    XINFERENCE_URL: str = Field(default="http://localhost:9997", description="xinference 服务地址")
...
class DBSettings(BaseSettings):
    """数据库配置"""
    SQLALCHEMY_DATABASE_URI: str = "sqlite:///./chatchat.db"
